# cc_chatbot.py - Chatbot wrapper for your Cybercrime Detector
...
# Load model and vectorizer (assumes model.pkl and vectorizer.pkl are in the same folder)
with open('model.pkl', 'rb') as f:
    model = pickle.load(f)
with open("vectorizer.pkl", "rb") as f:
    vectorizer = pickle.load(f)

# --- (Paste your law_map and high_risk_words here) ---
# For brevity, I'm using the same dictionaries you used in cc.py;
# paste the full law_map and high_risk_words content from your cc.py above here.
# --- BEGIN law_map & high_risk_words (copy the dictionaries from cc.py) ---
law_map = {
...
def translate_if_needed(text):
    try:
        ...
        lang = detected.lang if detected else 'en'
        if lang != 'en':
...
# -------------------- Streamlit Chat UI --------------------
st.set_page_config(page_title="CyberGuard Chatbot", layout="centered", page_icon="🛡️")
st.title("🛡️ CyberGuard — Cybercrime Chatbot")

st.sidebar.title("How to use")
st.sidebar.markdown("""
- Type or paste a suspicious message into the chat box.
- Or upload an image (OCR will extract text).
- The bot will analyze and reply with detection, laws, severity, and a downloadable report.
- View past analyses from the sidebar.
""")
...
if "messages" not in st.session_state:
    st.session_state.messages = [
        {"role": "assistant", "content": "Hello — I'm CyberGuard. Paste a suspicious message or upload an image and I'll analyze it for possible cybercrime."}
    ]
...
for msg in st.session_state.messages:
    with st.chat_message(msg["role"]):
        if msg.get("image"):
            st.image(msg["image"], caption=msg.get("image_caption", None), use_column_width=True)
        st.write(msg["content"])

# Image uploader (keeps conversation flow)
uploaded_file = st.file_uploader("Upload suspicious image (jpg, png)", type=["jpg", "jpeg", "png"])
if uploaded_file is not None:
    try:
        image = Image.open(uploaded_file)
        # display user image as a chat message
        st.session_state.messages.append({"role": "user", "content": "[Image uploaded]", "image": image, "image_caption": "User uploaded image"})
        # Extract text via OCR
        extracted = pytesseract.image_to_string(image)
        # also place the extracted text as user message for transparency
        st.session_state.messages.append({"role": "user", "content": f"Extracted text:\n{extracted}"})
        # re-render the new messages (Streamlit will show them on next rerun)
        st.experimental_rerun()
    except Exception as e:
        st.error(f"Error processing image: {e}")

# Chat input
if prompt := st.chat_input("Paste suspicious message or ask about a message..."):
    # add user message
    ...
    with st.spinner("Analyzing..."):
        translated_text, detected_lang = translate_if_needed(prompt)
        cleaned = clean_text(translated_text)
        try:
            vector = vectorizer.transform([cleaned])
            prediction = model.predict(vector)[0]
            # prediction probability: if model has predict_proba
            try:
                proba = model.predict_proba(vector)[0][1]
            except Exception:
                proba = None
        except Exception as e:
            st.error(f"Model/vectorizer error: {e}")
            prediction = False
            proba = None
        ...
        if prediction:
            confidence_text = f"Confidence: {proba:.0%}" if proba is not None else ""
            result = f"🚨 *Potential cybercrime detected.* {confidence_text}\n\n**Threat Level:** {severity}\n\n"
            if crimes:
                result += "**Detected crime patterns:** " + ', '.join(sorted(crimes)) + "\n\n"
            if laws:
                result += "**Possible laws/sections:** " + ', '.join(sorted(laws)) + "\n\n"
            result += "You can download a short analysis report or view the cleaned (translated) text below."
        else:
            confidence_text = f"Confidence: {(1-proba):.0%}" if proba is not None else ""
            result = f"✅ *No clear cybercrime patterns detected.* {confidence_text}\n\n"
            if crimes:
                result += "**Matched patterns (low confidence):** " + ', '.join(sorted(crimes)) + "\n\n"
            if laws:
                result += "**Possible laws detected:** " + ', '.join(sorted(laws)) + "\n\n"
            result += "If you're unsure, consider reporting to the authorities or sharing more context."
        ...
        # show cleaned text and time
        with st.expander("🧼 View Cleaned (English) Text"):
            st.code(cleaned)
        st.caption(f"Input language detected: {detected_lang.upper()}")

        st.subheader("📅 Analysis Time")
        st.write(datetime.now().strftime("%A, %d %B %Y - %I:%M %p"))

        # Report download
        report_bytes = generate_report(prompt, crimes, laws, severity)
        b64 = base64.b64encode(report_bytes).decode()
        href = f'<a href="data:file/txt;base64,{b64}" download="cybercrime_report.txt">📥 Download Report</a>'
        st.markdown(href, unsafe_allow_html=True)

        # Save history
        log_history(prompt, crimes, laws, severity, detected_lang)

# Footer / disclaimer
st.markdown("---")
st.caption("⚠ This tool provides a preliminary check. Always report real incidents to https://cybercrime.gov.in  \nPhishing SMS? Forward to 7726 (India)")
